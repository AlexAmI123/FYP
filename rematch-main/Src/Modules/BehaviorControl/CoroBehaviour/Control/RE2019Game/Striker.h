...
namespace RE2019
{
    ...
    void operator()(void)
    {
      ...
      if (commonSkills.isOurTeamKick()) // offense kickoff
      {
        ...
        while (true)
        {
          ...
          if (theKickoffState.allowedToScore)
            CR_EXIT_SUCCESS();
          else
            CR_YIELD();
        }
  ...
  private:
    ...
    Angle calcKickoffAngle()
    {
      ...
      Vector2f target = theRobotPose.toRobotCoordinates(targetOnField);
  ...
  CRBEHAVIOUR(StrikerSearchForBallTask)
  {
    ...
    void operator()(bool shouldBackUp = false)
    {
      ...
      // TODO - should choose another spot if nothing found after a rotation
  ...
  CRBEHAVIOUR(StrikerPlayingTask)
  {
    ...
    void operator()(void)
    {
      ...
      // kickoff = true; // FIXME - need to really figure out if it is kickoff or some other transition into playing state
