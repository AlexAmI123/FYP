/**
 * @file CoroBehaviorModule2022.cpp
 *
...
#include "CoroBehaviourModule2022.h"
...
#include "Modules/BehaviorControl/CoroBehaviour/Control/LikeCodeRelease/ExamplePositionForKickoff.h"
#include "Modules/BehaviorControl/CoroBehaviour/Control/LikeCodeRelease/ExampleStrikerPlayingState.h"

#include "Modules/BehaviorControl/CoroBehaviour/Control/RE2019Game/ReadyState.h"
#include "Modules/BehaviorControl/CoroBehaviour/Control/RE2019Game/PlayingState.h"

#include "Modules/BehaviorControl/CoroBehaviour/Control/RC2022VisualReferee/VisualRefereeControl.h"
#include "Modules/BehaviorControl/CoroBehaviour/Control/RC2022DynamicBallHandling/ReadyOrSetState.h"
#include "Modules/BehaviorControl/CoroBehaviour/Control/RC2022DynamicBallHandling/AttackerPlayingState.h"
#include "Modules/BehaviorControl/CoroBehaviour/Control/RC2022DynamicBallHandling/DefenderPlayingState.h"
...
DECL_TLOGGER(tlogger, "CoroBehaviourModule2022", TextLogging::INFO);

MAKE_MODULE(CoroBehaviourModule2022, behaviorControl, CoroBehaviourModule2022::getExtModuleInfo);

CoroBehaviourModule2022::CoroBehaviourModule2022()
      ...
      theCardRegistry(const_cast<ActivationGraph &>(theActivationGraph))
{
  ...
  TLOGD(tlogger, "Created CoroModuleBehaviourModule2022");
}

std::vector<ModuleBase::Info> CoroBehaviourModule2022::getExtModuleInfo()
{
  auto result = CoroBehaviourModule2022::getModuleInfo();
...
void CoroBehaviourModule2022::update(ActivationGraph &activationGraph)
{
  ...
  activationGraph.graph.emplace_back("CoroBehaviourModule2022", 0, TypeRegistry::getEnumName(status), theFrameInfo.time,
                                     theFrameInfo.time - statusStartTime, std::vector<std::string>());

#ifdef TARGET_SIM // TODO - not in 2021 BehaviorControl.cpp
  MODIFY("module:CoroBehaviourModule2022:status", status);
#endif
...
#define gotoState(s)  goto fsm_state_##s

// run s in the next cycle
#define nextState(s)  status = s; return




void CoroBehaviourModule2022::executeBehaviourTick()
{
...
void CoroBehaviourModule2022::checkUsbMountedButtonRequest()
{
...
void CoroBehaviourModule2022::checkJointTemperatureButtonRequest()
{
...
CoroBehaviour::BehaviourEnv::RepresentationRegistry CoroBehaviourModule2022::initRegisteredRepresentations()
{
  CoroBehaviour::BehaviourEnv::RepresentationRegistry representationRegistry;

#define CORO_REPRESENTATIONS_FOR_BEHAVIOUR
#include "CoroBehaviourRepresentations2022.inc"
...
void CoroBehaviourModule2022::registerEntryPointBehaviours()
{
  ...
  // LikeCodeRelease tasks needed by SoccerGameControlTask
  REGISTER_CORO(ExamplePositionForKickoffTask);
  REGISTER_CORO(ExampleStrikerPlayingStateTask);

  // RE2019 specific tasks
  REGISTER_CORO(RE2019::ReadyStateTask);
  REGISTER_CORO(RE2019::PlayingStateTask);

  // RoboCup 2022 technical challenges
  REGISTER_CORO(RC2022::VisualRefereeControlTask);

  // RC2022: dynamic ball handling challenge
  REGISTER_CORO(RC2022::ReadyOrSetStateTask);
  REGISTER_CORO(RC2022::AttackerPlayingStateTask);
  REGISTER_CORO(RC2022::DefenderPlayingStateTask);
