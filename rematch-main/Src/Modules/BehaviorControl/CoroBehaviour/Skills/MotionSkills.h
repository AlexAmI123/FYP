...
namespace CoroBehaviour
{
  ...
  // CRBEHAVIOUR(WalkToPoseOnFieldTask)
  // {
  //   CRBEHAVIOUR_INIT(WalkToPoseOnFieldTask) {}

  //   void operator()(const Pose2f &targetOnField, const Pose2f &speed,
  //                   const MotionRequest::ObstacleAvoidance &obstacleAvoidance, bool keepTargetRotation,
  //                   float positionThreshold, Angle angleThreshold = 180_deg)
  //   {
  //     Pose2f target = theRobotPose.inversePose + targetOnField;

  //     CRBEHAVIOUR_LOOP()
  //     {
  //       walkToPoseTask(target, speed, obstacleAvoidance, keepTargetRotation, positionThreshold, angleThreshold);

  //       if (walkToPoseTask.isSuccess())
  //         CR_EXIT_SUCCESS();
  //       else
  //         CR_YIELD();
  //     }
