...
DECL_TLOGGER(tlogger, "TeamMessageHandler2023b", TextLogging::INFO);
...
void TeamMessageHandler2023b::update(TeamMessage2023OutputGenerator& outputGenerator)
{
  ...
  // TODO: is this needed/used anywhere?
  // if (outputGenerator.teamMessage.isUpgright)
  //   outputGenerator.teamMessage.frameTimeWhenLastUpright = theFrameInfo.time;
...
void TeamMessageHandler2023b::generateOutputMessage(TeamMessage2023OutputGenerator& outputGenerator) const
{
  ...
  // isUpright was already filled out in update method - not sure why it's done this way
...
void TeamMessageHandler2023b::checkOutputMessageBytes(const TeamMessage2023 &originalMessage,
                                                      const TeamMessageBytes &teamMessageBytes) const
{
  BitStream::Reader reader(teamMessageBytes.data, teamMessageBytes.size);
  TeamMessage2023 decodedMessage;
                                               ...
                                               const TeamMessageBytes &teamMessageBytes)
{
  ...
  if ((outputGenerator.sentMessages == 1) || (outputGenerator.sentMessages % 10 == 0))
    TLOGI(tlogger, "have now sent {} messages from {}, latest with timestamp {}, frameTimeSent {}",
          outputGenerator.sentMessages, theRobotInfo.number, outputGenerator.teamMessage.timestamp, theFrameInfo.time);
  else
    TLOGD(tlogger, "sent msg from {}, sentMessages {}, timestamp {}, frameTimeSent {}", theRobotInfo.number,
...
void TeamMessageHandler2023b::update(TeamData& teamData)
{
  ...
  for (unsigned iMessage = 1; !inputTeamMessageBytesBuffer.empty(); iMessage++)
  {
    ...
    if (readInputMessageBytes(inputMessageBytes, receivedTeamMessage))
    {
      ...
      if ((teamData.receivedMessages == 1) || (teamData.receivedMessages % 10 == 0))
        TLOGI(tlogger, "have now received {} messages, latest from {} (teammateMsgCount {}), timestamp {}",
        ...
        TLOGD(tlogger, "received msg from {}, timestamp {}, msgCount teamate/total {}/{}",
  ...
  // if (!wasEmpty)
  //   TLOGD(tlogger, "inputTeamMessageBytesBuffer size {} empty {}", inputTeamMessageBytesBuffer.size(),
